...
exports.exhibitions = exports.getAISuggestions = exports.getExhibitionByShareCode = exports.publishExhibition = exports.deleteExhibition = exports.getExhibitionList = exports.getExhibitionWithPaintings = exports.getExhibition = exports.updateExhibition = exports.createExhibition = exports.getThemeSuggestions = void 0;
...
const generateAISuggestions = (sections, title, introduction) => {
    const suggestions = [];
    let suggestionId = 0;
    ...
    for (const section of sections) {
        ...
        for (const item of section.items) {
            const painting = index_1.paintings.find(p => p.id === item.paintingId);
            if (!painting)
                continue;
            ...
            const painter = index_1.painters.find(p => p.id === painting.painterId);
            const dynasty = index_1.dynasties.find(d => d.id === painting.dynastyId);
    ...
    const themes = new Set(allItems.map(item => {
        const p = index_1.paintings.find(p => p.id === item.paintingId);
        return p?.theme;
    }).filter(Boolean));
    if (themes.size >= 3) {
        suggestions.push({
...
const checkChronologicalOrder = (items) => {
    const paintingYears = items.map(item => {
        const painting = index_1.paintings.find(p => p.id === item.paintingId);
        const dynasty = painting ? index_1.dynasties.find(d => d.id === painting.dynastyId) : null;
        let year = 1000;
        if (painting?.dynastyId === 'wei-jin')
            year = 400;
        else if (painting?.dynastyId === 'tang')
            year = 700;
        else if (painting?.dynastyId === 'wu-dai')
            year = 950;
        else if (painting?.dynastyId === 'song')
            year = 1100;
        else if (painting?.dynastyId === 'yuan')
            year = 1350;
        else if (painting?.dynastyId === 'ming')
            year = 1550;
        else if (painting?.dynastyId === 'qing')
            year = 1700;
        return {
            id: item.paintingId,
            year,
...
exports.getThemeSuggestions = getThemeSuggestions;
const createExhibition = (request) => {
    const now = Date.now();
    const exhibition = {
        id: (0, uuid_1.v4)(),
        ...request,
...
const updateExhibition = (id, request) => {
...
const getExhibitionWithPaintings = (id) => {
    ...
    const sections = exhibition.sections.map(section => ({
        ...
        items: section.items.map(item => ({
            ...item,
            painting: index_1.paintings.find(p => p.id === item.paintingId)
        }))
    }));
    return {
        ...exhibition,
...
exports.getExhibitionWithPaintings = getExhibitionWithPaintings;
const getExhibitionList = () => {
    return exhibitions.map(exhibition => {
        ...
        const coverPainting = exhibition.coverPaintingId
            ? index_1.paintings.find(p => p.id === exhibition.coverPaintingId)
            : index_1.paintings.find(p => p.id === allItems[0]?.paintingId);
        return {
            id: exhibition.id,
...
exports.getExhibitionList = getExhibitionList;
...
const publishExhibition = (id) => {
    const exhibition = exhibitions.find(e => e.id === id);
    if (!exhibition)
        return null;
    exhibition.isPublished = true;
    exhibition.shareCode = (0, uuid_1.v4)().slice(0, 8).toUpperCase();
    exhibition.updatedAt = Date.now();
    return exhibition;
};
exports.publishExhibition = publishExhibition;
const getExhibitionByShareCode = (shareCode) => {
    return exhibitions.find(e => e.shareCode === shareCode.toUpperCase()) || null;
};
exports.getExhibitionByShareCode = getExhibitionByShareCode;
const getAISuggestions = (sections, title, introduction) => {
