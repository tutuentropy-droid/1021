...
router.get('/paintings/:id/deep-analysis', (req, res) => {
    const painting = data_1.paintings.find(p => p.id === req.params.id);
    if (!painting) {
        res.status(404).json({ error: '画作不存在' });
        return;
...
router.get('/knowledge-graph', (req, res) => {
    ...
    const addPainterNode = (painterId) => {
        if (nodes.has(painterId))
            return;
        const painter = data_1.painters.find(p => p.id === painterId);
        if (painter) {
            nodes.set(painterId, {
                id: painterId,
                type: 'painter',
    ...
    const addSchoolNode = (schoolId) => {
        if (nodes.has(schoolId))
            return;
        const school = data_1.schools.find(s => s.id === schoolId);
        if (school) {
            nodes.set(schoolId, {
                id: schoolId,
                type: 'school',
    ...
    const addPaintingNode = (paintingId) => {
        if (nodes.has(paintingId))
            return;
        const painting = data_1.paintings.find(p => p.id === paintingId);
        if (painting) {
    ...
    const addDynastyNode = (dynastyId) => {
        if (nodes.has(dynastyId))
            return;
        const dynasty = data_1.dynasties.find(d => d.id === dynastyId);
        if (dynasty) {
            nodes.set(dynastyId, {
                id: dynastyId,
                type: 'dynasty',
    ...
    const buildFromPainter = (pid, currentDepth) => {
        if (currentDepth > graphDepth)
            return;
        const painter = data_1.painters.find(p => p.id === pid);
        if (!painter)
            return;
        addPainterNode(pid);
        ...
        painter.teacherIds?.forEach(tid => {
            ...
            if (currentDepth < graphDepth)
                buildFromPainter(tid, currentDepth + 1);
        });
        painter.studentIds?.forEach(sid => {
            ...
            if (currentDepth < graphDepth)
                buildFromPainter(sid, currentDepth + 1);
        });
        ...
        data_1.paintings.filter(p => p.painterId === pid).forEach(p => {
    ...
    const buildFromSchool = (sid) => {
        const school = data_1.schools.find(s => s.id === sid);
        if (!school)
            return;
        addSchoolNode(sid);
        ...
        data_1.painters.filter(p => p.schoolIds.includes(sid)).forEach(p => {
            addPainterNode(p.id);
            edges.push({
    ...
    const buildFromPainting = (pid) => {
        const painting = data_1.paintings.find(p => p.id === pid);
        if (!painting)
            return;
        addPaintingNode(pid);
...
function generateSpatialLayout(painting) {
    const templates = {
...
function generateBrushworkQuality(painting) {
    const templates = {
...
function generateSealsAndInscriptions(painting) {
    const templates = {
        'fuchun-shanju-tu': [
            { type: 'inscription', owner: '黄公望', content: '至正七年，仆归富春山居，无用师偕往。暇日于南楼援笔写成此卷，兴之所至，不觉亹亹布置如许，逐旋填札，阅三四载未得完备，盖因留在山中，而云游在外故尔。今特取回行李中，早晚得暇，当为着笔。无用过虑有巧取豪敚者，俾先识卷末，庶使知其成就之难也。十年，青龙在庚寅，歜节前一日，大痴学人书于云间夏氏知止堂。', meaning: '黄公望自题创作始末，说明此画是为友人无用师所作', position: '卷尾', dynasty: '元' },
            { type: 'seal', owner: '黄公望', content: '黄氏子久', meaning: '作者名号章', position: '卷尾题跋处', dynasty: '元' },
            { type: 'seal', owner: '黄公望', content: '一峰道人', meaning: '作者别号章', position: '卷尾', dynasty: '元' },
            { type: 'inscription', owner: '沈周', content: '今观黄子久《富春山居图》，笔墨浑厚，丘壑幽邃，峰峦起伏，云山烟树，沙汀村舍，一一具于目前。诚为画中兰亭，信不虚矣。', meaning: '沈周跋语，盛赞此画为"画中兰亭"', position: '卷尾', dynasty: '明' },
            { type: 'seal', owner: '乾隆', content: '乾隆御览之宝', meaning: '乾隆帝鉴藏玺印', position: '卷首', dynasty: '清' },
            { type: 'seal', owner: '乾隆', content: '石渠宝笈', meaning: '内府收藏著录章', position: '卷中', dynasty: '清' },
            { type: 'seal', owner: '吴湖帆', content: '吴湖帆珍藏', meaning: '近代收藏家吴湖帆鉴藏印', position: '"剩山图"段', dynasty: '近现代' }
        ],
        'qingming-shanghe-tu': [
            { type: 'inscription', owner: '李东阳', content: '宋家汴都全盛时，万方玉帛梯航随。清明上河俗所尚，倾城仕女携童儿。...', meaning: '明代李东阳题跋，咏叹汴京繁华', position: '卷尾', dynasty: '明' },
            { type: 'seal', owner: '宣和', content: '宣和中秘', meaning: '宋徽宗朝内府收藏印', position: '卷首', dynasty: '北宋' },
            { type: 'seal', owner: '项元汴', content: '天籁阁', meaning: '明代大收藏家项元汴鉴藏印', position: '卷中多处', dynasty: '明' },
            { type: 'seal', owner: '毕沅', content: '毕沅审定', meaning: '清代毕沅、毕泷兄弟收藏印', position: '卷尾', dynasty: '清' },
            { type: 'seal', owner: '溥仪', content: '宣统御览之宝', meaning: '宣统帝溥仪钤印，此画曾被溥仪携出宫外', position: '卷尾', dynasty: '清' }
        ]
    };
    return templates[painting.id] || [
        { type: 'inscription', owner: '作者自题', content: '画家自题款识与创作年月', meaning: '作者署名及创作信息', position: '边角处', dynasty: '本朝' },
        { type: 'seal', owner: '作者', content: '姓名章、字号章', meaning: '作者身份标识', position: '题跋下方', dynasty: '本朝' },
        { type: 'seal', owner: '历代内府', content: '御览之宝、石渠宝笈等', meaning: '皇家收藏鉴藏印', position: '卷首、卷中', dynasty: '各代' },
        { type: 'seal', owner: '历代收藏家', content: '各家鉴藏印', meaning: '见证画作流传经过', position: '画面各处', dynasty: '各代' }
    ];
}
function generateTransmissionHistory(painting) {
    const templates = {
...
function generateScholarlyAppreciation(painting) {
    const templates = {
...
router.get('/roleplay-scenarios', (req, res) => {
    const result = data_1.scenarios.map(s => ({
        id: s.id,
        title: s.title,
        dynasty: s.dynasty,
        era: s.era,
...
router.get('/roleplay-scenarios/:id', (req, res) => {
    const scenario = data_1.scenarios.find(s => s.id === req.params.id);
    if (!scenario) {
...
router.get('/roleplay-scenarios/:id/choices/:choiceId', (req, res) => {
    const scenario = data_1.scenarios.find(s => s.id === req.params.id);
    if (!scenario) {
        res.status(404).json({ error: '场景不存在' });
        return;
...
router.get('/roleplay-scenarios/:id/consequences/:consequenceId', (req, res) => {
    const scenario = data_1.scenarios.find(s => s.id === req.params.id);
    if (!scenario) {
        res.status(404).json({ error: '场景不存在' });
        return;
...
router.post('/roleplay-result', (req, res) => {
    const { scenarioId, path } = req.body;
    const scenario = data_1.scenarios.find(s => s.id === scenarioId);
    if (!scenario) {
        res.status(404).json({ error: '场景不存在' });
        return;
    }
    ...
    const consequenceIds = path.map((p) => p.consequenceId);
    const allConsequences = consequenceIds
        .map((cid) => scenario.consequences.find(c => c.id === cid))
        .filter(Boolean);
    ...
    if (scenarioId === 'southern-song-court') {
        const hasShiqi = consequenceIds.some((id) => id.includes('1-d') || id.includes('2-b') || id.includes('3-b'));
        const hasXingsi = consequenceIds.some((id) => id.includes('3-c'));
        const hasZhengzhi = consequenceIds.some((id) => id.includes('2-c'));
        ...
        const hasYige = consequenceIds.some((id) => id.includes('3-a'));
        const hasShufa = consequenceIds.some((id) => id.includes('3-b'));
        const hasXiesheng = consequenceIds.some((id) => id.includes('3-c'));
        const hasJiecao = consequenceIds.some((id) => id.includes('2-b'));
        ...
        const hasRonghe = consequenceIds.some((id) => id.includes('1-b') || id.includes('1-d') || id.includes('2-c'));
        const hasXiesheng = consequenceIds.some((id) => id.includes('1-c'));
        const hasJiecao = consequenceIds.some((id) => id.includes('3-b'));
        const hasFansi = consequenceIds.some((id) => id.includes('2-b'));
